...
def screpeing_profile_post(user_name,cookies,reaction_limite,viwes_limite,comment_limite,nb_tweets):  #seach fast page and get data
    # try:
        ...
        if "invaid" in rest_id:
            ...
            with open(f'scraped/data{random.randint(1,100)}.json',"w",encoding="utf-8") as f:
                json_data = json.dump(tweet_resp.json() ,f ,ensure_ascii=False, indent=4)
                f.flush()
                f.close()

            tweet_text = tweet_resp.text
...
def loping_serch(tweet_text,cookies,reaction_limite,viwes_limite,comment_limite,nb_tweets,chatagory,headers):
    ...
    try:
        ...
        response = requests.get('https://twitter.com/i/api/graphql/PaIcTAgMdfWySgs3aVU5TA/SearchTimeline',
